...
class RaceParticipantTest {
    ...
    @Test
    fun raceParticipant_RaceCanceled_ProgressUpdated() = runTest {
        launch { raceParticipant.run() }
        advanceTimeBy(raceParticipant.maxProgress * (raceParticipant.progressDelayMillis / 2))
        runCurrent()
        raceParticipant.reset()
        assertEquals(raceParticipant.currentProgress, 0)
    }

    @Test
    fun raceParticipant_RaceHalfProgress_ProgressUpdated() = runTest {
        launch { raceParticipant.run() }
        advanceTimeBy(raceParticipant.maxProgress * (raceParticipant.progressDelayMillis / 2))
        runCurrent()
        assertEquals(raceParticipant.currentProgress, 50)
    }
